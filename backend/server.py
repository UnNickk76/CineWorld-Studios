...
async def get_my_statistics(user: dict = Depends(get_current_user)):
    ...
    # Box office revenue (stats only - total revenue generated by films)
    total_box_office = sum(f.get('total_revenue', 0) for f in films)
    ...
    # Calculate actual spending and earnings based on reality
    total_spent = total_film_costs + total_infra_costs
    
    # Starting funds for new users is $5M
    INITIAL_FUNDS = 5000000
    
    # Current funds tell the real story
    current_funds = user.get('funds', 0)
    
    # Real profit = current funds - initial funds
    # This represents actual net gain/loss from playing
    real_profit = current_funds - INITIAL_FUNDS
    
    # Total earned = what user has actually received (funds change + spending)
    # total_earned = current_funds + total_spent - INITIAL_FUNDS
    # Simplified: if user started with 5M, spent X, and has Y now, they earned Y + X - 5M
    total_earned = current_funds + total_spent - INITIAL_FUNDS
    if total_earned < 0:
        total_earned = lifetime_collected if lifetime_collected > 0 else total_box_office + total_infra_revenue
    
    return {
        'total_films': len(films),
        'total_revenue': total_box_office,  # Box office stats
        ...
        'profit_loss': real_profit,  # Actual profit/loss from initial $5M
