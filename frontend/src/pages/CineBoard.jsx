...
const CineBoard = () => {
  ...
  const [socialFilms, setSocialFilms] = useState([]);
  const [socialPage, setSocialPage] = useState(1);
  const [socialTotal, setSocialTotal] = useState(0);
  const [socialLoading, setSocialLoading] = useState(false);
  const [topLiked, setTopLiked] = useState([]);
  const [myBonuses, setMyBonuses] = useState(null);

  // Load social feed
  useEffect(() => {
    if (currentView === 'social-feed') {
      setSocialLoading(true);
      api.get(`/social/feed?page=${socialPage}&limit=200`)
        .then(r => { setSocialFilms(r.data.films || []); setSocialTotal(r.data.total || 0); })
        .catch(() => {})
        .finally(() => setSocialLoading(false));
      api.get('/social/my-bonuses').then(r => setMyBonuses(r.data)).catch(() => {});
    } else if (currentView === 'top-liked') {
      setSocialLoading(true);
      api.get('/social/top-liked')
        .then(r => setTopLiked(r.data.films || []))
        .catch(() => {})
        .finally(() => setSocialLoading(false));
    }
  }, [api, currentView, socialPage]);

  const handleSocialLike = async (filmId) => {
    try {
      const res = await api.post(`/films/${filmId}/like`);
      // Update in social feed
      setSocialFilms(prev => prev.map(f => f.id === filmId ? { ...f, user_liked: res.data.liked, likes_count: res.data.likes_count } : f));
      // Update in top-liked
      setTopLiked(prev => prev.map(f => f.id === filmId ? { ...f, user_liked: res.data.liked, likes_count: res.data.likes_count } : f));
      // Update in regular films
      setFilms(prev => prev.map(f => f.id === filmId ? { ...f, user_liked: res.data.liked, likes_count: res.data.likes_count } : f));
    } catch (e) {
  ...
  if (loading && !['social-feed', 'top-liked'].includes(currentView)) return <LoadingSpinner />;

  // ═══════════════ SOCIAL FEED VIEW ═══════════════
  if (currentView === 'social-feed') {
    return (
      <div className="pt-14 pb-20 px-2 sm:px-3 max-w-[1600px] mx-auto" data-testid="social-feed-page">
        {/* Header */}
        <div className="flex items-center gap-2.5 mb-3">
          <Heart className="w-6 h-6 text-pink-500" />
          <div className="flex-1">
            <h1 className="font-['Bebas_Neue'] text-2xl text-pink-400">CineBoard Social</h1>
          ...
          {myBonuses && (
            <div className="flex gap-2 flex-shrink-0">
              <div className="text-center px-2 py-1 rounded-lg bg-pink-500/10 border border-pink-500/20">
                <p className="text-[10px] text-pink-400 font-bold">{myBonuses.likes_given}</p>
                <p className="text-[7px] text-gray-500">dati</p>
              </div>
              <div className="text-center px-2 py-1 rounded-lg bg-red-500/10 border border-red-500/20">
                <p className="text-[10px] text-red-400 font-bold">{myBonuses.likes_received}</p>
                <p className="text-[7px] text-gray-500">ricevuti</p>
              </div>
              <div className="text-center px-2 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <p className="text-[10px] text-emerald-400 font-bold">+{myBonuses.giver_bonus}%</p>
                <p className="text-[7px] text-gray-500">bonus</p>
              </div>
            </div>
          )}
        </div>

        <p className="text-[9px] text-gray-600 mb-2">{socialTotal} film</p>

        {socialLoading ? (
          <div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 text-pink-400 animate-spin" /></div>
        ) : socialFilms.length === 0 ? (
          <Card className="bg-[#111113] border-white/5 p-8 text-center">
            <Film className="w-10 h-10 text-gray-700 mx-auto mb-2" />
            <p className="text-sm text-gray-500">Nessun film ancora</p>
          </Card>
        ) : (
          <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(min(85px, 11vw), 1fr))' }} data-testid="social-film-grid">
            {socialFilms.map(f => (
              <div key={f.id} className="group relative rounded overflow-hidden bg-[#0d0d0f] border border-white/5 hover:border-pink-500/30 transition-all" data-testid={`social-film-${f.id}`}>
                {/* Poster - clickable */}
                <div className="aspect-[2/3] relative cursor-pointer" onClick={() => navigate(`/films/${f.id}`)}>
                  {f.poster_url ? (
                    <img src={f.poster_url.startsWith('/') ? `${BACKEND_URL}${f.poster_url}` : f.poster_url} alt={f.title} loading="lazy"
                  ...
                  {/* Studio name overlay at top */}
                  <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/80 to-transparent px-1 pt-0.5 pb-2">
                    <p className="text-[6px] text-white/80 truncate leading-tight">{f.studio_name}</p>
                  </div>
                  {/* Like button overlay at bottom */}
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-1 pb-0.5 pt-2 flex items-center justify-between">
                    <button
                      className={`flex items-center gap-0.5 transition-colors ${f.user_liked ? 'text-red-400' : 'text-white/60 hover:text-red-400'}`}
                      onClick={(e) => { e.stopPropagation(); handleSocialLike(f.id); }}
                      data-testid={`social-like-${f.id}`}
                    >
                      <Heart className={`w-2.5 h-2.5 ${f.user_liked ? 'fill-red-400' : ''}`} />
                      <span className="text-[7px] font-bold">{f.likes_count || 0}</span>
                    </button>
                    {f.quality_score != null && (
                      <span className="text-[7px] text-yellow-400 font-bold">{Math.round(f.quality_score)}%</span>
                    )}
                  </div>
                </div>
                {/* Title under poster */}
                <div className="px-0.5 py-0.5">
                  <p className="text-[7px] font-semibold text-white truncate leading-tight">{f.title}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  // ═══════════════ TOP LIKED LEADERBOARD VIEW ═══════════════
  if (currentView === 'top-liked') {
    return (
      <div className="pt-14 pb-20 px-3 max-w-4xl mx-auto" data-testid="top-liked-page">
        <div className="flex items-center gap-3 mb-4">
          <Heart className="w-7 h-7 text-red-500 fill-red-500" />
          <div>
            <h1 className="font-['Bebas_Neue'] text-2xl text-red-400">Top Liked</h1>
            <p className="text-xs text-gray-400">{language === 'it' ? 'I film più amati dalla community' : 'Most loved films by the community'}</p>
          </div>
        </div>
        ...
        {socialLoading ? (
          ...
          <Card className="bg-[#111113] border-white/5 p-8 text-center">
            ...
            <p className="text-sm text-gray-500">{language === 'it' ? 'Nessun film con like' : 'No liked films yet'}</p>
            <p className="text-xs text-gray-600 mt-1">{language === 'it' ? 'Metti like nella sezione Social!' : 'Like films in the Social section!'}</p>
          </Card>
        ) : (
          <div className="space-y-2">
            {topLiked.map(f => (
              <Card key={f.id} className={`bg-[#111113] border-white/5 overflow-hidden ${f.rank <= 3 ? 'border-red-500/20' : ''}`} data-testid={`top-liked-${f.rank}`}>
                <div className="flex">
                  <div className={`w-10 flex items-center justify-center font-bold text-sm ${
                    f.rank === 1 ? 'bg-yellow-500 text-black' : f.rank === 2 ? 'bg-gray-300 text-black' : f.rank === 3 ? 'bg-amber-600 text-white' : f.rank <= 10 ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-gray-400'
                  }`}>#{f.rank}</div>
                  <div className="w-14 flex-shrink-0 cursor-pointer" onClick={() => navigate(`/films/${f.id}`)}>
                    {f.poster_url ? (
                      <img src={f.poster_url.startsWith('/') ? `${BACKEND_URL}${f.poster_url}` : f.poster_url} alt="" className="w-full h-full object-cover" loading="lazy" />
                    ) : (
                      <div className="w-full h-full bg-gray-900 flex items-center justify-center"><Film className="w-4 h-4 text-gray-700" /></div>
                    )}
                  </div>
                  <CardContent className="flex-1 p-2.5">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-xs truncate cursor-pointer hover:text-red-400" onClick={() => navigate(`/films/${f.id}`)}>{f.title}</h3>
                        <p className="text-[9px] text-gray-500 truncate">{f.studio_name}</p>
                      </div>
                      <button
                        className={`flex items-center gap-1 px-2 py-1 rounded-full transition-all ${f.user_liked ? 'bg-red-500/20 text-red-400' : 'bg-white/5 text-gray-400 hover:bg-red-500/10 hover:text-red-400'}`}
                        onClick={() => handleSocialLike(f.id)}
                        data-testid={`top-liked-like-${f.id}`}
                      >
                        <Heart className={`w-3 h-3 ${f.user_liked ? 'fill-red-400' : ''}`} />
                        <span className="text-xs font-bold">{f.likes_count}</span>
                      </button>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      {f.genre && <Badge className="bg-white/10 text-gray-300 text-[8px] h-4">{f.genre}</Badge>}
                      {f.quality_score != null && <span className="text-[9px] text-yellow-400">{Math.round(f.quality_score)}%</span>}
                      <span className="text-[8px] text-emerald-400 ml-auto">+{f.like_bonus}% bonus</span>
                    </div>
                  </CardContent>
                </div>
              </Card>
