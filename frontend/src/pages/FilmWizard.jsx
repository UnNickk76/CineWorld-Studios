  ...
  const renderStep = () => {
    switch(step) {
      case 1: return (<div className="space-y-3">
        ...
        <div><Label className="text-xs">{language === 'it' ? 'Settimane' : 'Weeks'}: {filmData.weeks_in_theater}</Label><Slider value={[filmData.weeks_in_theater]} onValueChange={([v])=>setFilmData({...filmData,weeks_in_theater:v})} min={1} max={4} /></div>
      </div>);
      ...
      case 12:
        ...
        return (<Card className="bg-[#1A1A1A] border-white/10"><CardHeader className="pb-2"><CardTitle className="font-['Bebas_Neue'] text-xl">{filmData.title}</CardTitle><CardDescription>{t(filmData.genre)} • {filmData.weeks_in_theater}w</CardDescription></CardHeader><CardContent className="space-y-3">
          ...
          <div className="pt-2 border-t border-white/10 text-sm space-y-1">
            ...
            <div className="flex justify-between"><span className="text-gray-400">{language === 'it' ? 'Costo Attrezzatura' : 'Equipment'}</span><span className="text-red-400">-${((equipment.find(e=>e.name===filmData.equipment_package)||{cost:0}).cost).toLocaleString()}</span></div>
            ...
            <div className="flex justify-between"><span className="text-gray-400">{language === 'it' ? 'Costo Location' : 'Location'}</span><span className="text-red-400">-${(filmData.locations.reduce((s,l)=>{const lo=locations.find(x=>x.name===l);return s+(lo?lo.cost_per_day*(filmData.location_days[l]||7):0);},0)).toLocaleString()}</span></div>
