...
import {
  ...
  Scissors, Eye, Sparkles, Clock, Flame, Award, Gift, Shield
} from 'lucide-react';
import {
...
function StatusDot({ status }) {
  const colors = { playing: 'bg-cyan-400', in_vs: 'bg-purple-400', idle: 'bg-green-400' };
  const labels = { playing: 'In gioco', in_vs: 'In VS', idle: 'Online' };
  return (
...
export default function MiniGamesPage() {
  ...
  const loadGames = useCallback(async () => {
    try { const r = await api.get('/arcade/games'); setGames(r.data); } catch {}
  }, [api]);

  const loadStats = useCallback(async () => {
    try { const r = await api.get('/arcade/solo/stats'); setStats(r.data); } catch {}
  }, [api]);
  ...
  const loadStreak = useCallback(async () => {
    try { const r = await api.get('/arcade/streak'); setStreak(r.data); } catch {}
  }, [api]);

  const loadTitles = useCallback(async () => {
    try { const r = await api.get('/arcade/titles'); setTitles(r.data); } catch {}
  }, [api]);
  ...
  const loadLeaderboard = useCallback(async (gid, per) => {
    try { const r = await api.get(`/arcade/leaderboard/${gid}?period=${per}`); setLeaderboard(r.data); } catch {}
  }, [api]);

  const loadGlobalLb = useCallback(async () => {
    try { const r = await api.get('/arcade/leaderboard-global'); setGlobalLb(r.data); } catch {}
  }, [api]);
  ...
  const setGameStatus = async (status) => {
    try { await api.post('/arcade/status', { status }); } catch {}
  };

  const startSolo = (gameId) => {
    setPlaying(gameId);
    setLastResult(null);
  ...
  const onSoloComplete = async (score) => {
    ...
    try {
      ...
      if (res.data.reward) {
        ...
        const parts = [];
        if (r.hype) parts.push(`+${r.hype} hype`);
        if (r.xp) parts.push(`+${r.xp} XP`);
        if (r.funds) parts.push(`+$${r.funds.toLocaleString()}`);
  ...
  const onVsCreatorComplete = async (score) => {
    try {
      ...
      setGameStatus('idle');
      toast.success(`Punteggio ${score} registrato!`);
      loadVs();
  ...
  const onVsOpponentComplete = async (score) => {
    ...
    try {
      ...
      loadVs();
      loadStreak();
      if (res.data.streak?.milestone_reward) {
        toast.success(`Streak x${res.data.streak.streak}! +$${res.data.streak.milestone_reward.toLocaleString()} bonus!`);
      }
    } catch (e) { toast.error(e.response?.data?.detail || 'Errore'); }
  };

  const closeVs = () => {
    setVsPlaying(null); setVsChallengeId(null); setVsRole(null); setVsResult(null);
    setGameStatus('idle');
  };
  ...
  if (playing) {
    ...
    return (
      <div className="max-w-md mx-auto px-3 pt-3 pb-24" data-testid="solo-playing">
        ...
        {!lastResult ? (
          ...
          <Card className="bg-[#1A1A1B] border-gray-800">
            <CardContent className="p-5 text-center space-y-3">
              ...
              {lastResult.new_title && (
                <div className="flex items-center justify-center gap-1 text-xs text-yellow-400">
                  <Award className="w-3.5 h-3.5" /> Titolo sbloccato: {lastResult.new_title}
                </div>
              )}
  ...
  if (vsPlaying) {
    ...
    return (
      <div className="max-w-md mx-auto px-3 pt-3 pb-24" data-testid="vs-playing">
        <div className="flex items-center justify-between mb-2">
        ...
        {!vsResult ? (
          ...
          <Card className="bg-[#1A1A1B] border-gray-800">
            <CardContent className="p-5 text-center space-y-3">
              {vsResult.winner_id === user?.id ? (
                <Trophy className="w-10 h-10 mx-auto text-yellow-400" />
              ) : vsResult.winner_id === 'draw' ? (
                <Shield className="w-10 h-10 mx-auto text-gray-400" />
              ) : (
                <Trophy className="w-10 h-10 mx-auto text-red-400" />
              )}
              <p className="text-xl font-black">
                {vsResult.winner_id === user?.id ? 'HAI VINTO!' : vsResult.winner_id === 'draw' ? 'PAREGGIO!' : 'HAI PERSO!'}
              </p>
              ...
              {vsResult.pot > 0 && vsResult.winner_id === user?.id && <p className="text-sm text-green-400 font-bold">+${vsResult.pot.toLocaleString()}</p>}
              {vsResult.streak && vsResult.streak.streak >= 3 && (
                <div className="flex items-center justify-center gap-1 text-xs text-orange-400">
                  <Flame className="w-3.5 h-3.5" /> Streak x{vsResult.streak.streak}!
                  {vsResult.streak.milestone_reward && <span className="text-green-400 ml-1">+${vsResult.streak.milestone_reward.toLocaleString()}</span>}
                </div>
              )}
              <Button variant="outline" className="border-gray-700 h-8 text-xs" onClick={closeVs}>Torna</Button>
            </CardContent>
  ...
  // === MAIN PAGE ===
  return (
    <div className="max-w-lg mx-auto px-3 pt-2 pb-24 space-y-2" data-testid="minigiochi-page">
      {/* Header with streak */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Gamepad2 className="w-5 h-5 text-cyan-400" />
          <h1 className="text-lg font-bold">Minigiochi</h1>
        </div>
        <div className="flex items-center gap-2">
      ...
      {titles.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
          {titles.map(t => (
            <span key={t.game_id} className="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-[9px] text-yellow-400 font-medium">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="w-full grid grid-cols-4 h-8">
          <TabsTrigger value="solo" className="text-[10px]" data-testid="tab-solo"><Play className="w-3 h-3 mr-0.5" />Solo</TabsTrigger>
          <TabsTrigger value="vs" className="text-[10px]" data-testid="tab-vs"><Swords className="w-3 h-3 mr-0.5" />VS</TabsTrigger>
          <TabsTrigger value="classifica" className="text-[10px]" data-testid="tab-classifica"><Crown className="w-3 h-3 mr-0.5" />Top</TabsTrigger>
          <TabsTrigger value="stats" className="text-[10px]" data-testid="tab-stats"><BarChart3 className="w-3 h-3 mr-0.5" />Stats</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="solo">
          <div className="grid grid-cols-2 gap-2">
            {games.map(g => {
              ...
              return (
                <Card key={g.id}
                  className={`bg-[#1A1A1B] border-gray-800 active:scale-[0.97] transition-all ${g.bonus ? 'border-yellow-500/10' : ''}`}
                  ...
                  <CardContent className="p-2.5 space-y-1">
                    ...
                    {st ? (
                      <p className="text-[9px] text-cyan-400">Best: {st.best} | {st.plays}x</p>
                    ) : (
                      <p className="text-[9px] text-gray-600">Mai giocato</p>
                    )}
                  </CardContent>
                </Card>
              );
        ...
        <TabsContent value="vs">
            ...
            <Card className="bg-gradient-to-r from-orange-500/10 to-red-500/10 border-orange-500/20 mb-2">
              <CardContent className="p-2.5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold">Streak: {streak.streak}</p>
                    <p className="text-[9px] text-gray-400">Record: {streak.best_streak} | {streak.wins}W {streak.losses}L</p>
                  </div>
                </div>
          ...
          <Tabs value={vsTab} onValueChange={setVsTab}>
            <TabsList className="w-full grid grid-cols-3 h-7 mb-2">
              ...
              <TabsTrigger value="open" className="text-[10px]" data-testid="vs-open-tab">
                Aperte{pendingVs.length > 0 && <Badge className="ml-1 bg-cyan-500 text-[8px] px-1 h-3.5">{pendingVs.length}</Badge>}
              </TabsTrigger>
            ...
            <TabsContent value="create">
              <div className="grid grid-cols-2 gap-1.5">
                {games.map(g => {
                  ...
                  return (
                    <Card key={g.id}
                      className="bg-gradient-to-br from-cyan-500/5 to-purple-500/5 border-cyan-500/15 active:scale-[0.97] transition-all"
            ...
            <TabsContent value="open">
              {pendingVs.length === 0 ? (
                <Card className="bg-[#1A1A1B] border-gray-800"><CardContent className="p-6 text-center text-gray-500">
                  <Swords className="w-6 h-6 mx-auto mb-1.5 opacity-30" /><p className="text-xs">Nessuna sfida aperta</p>
                </CardContent></Card>
              ) : pendingVs.map(vs => (
                <Card key={vs.id} className="bg-[#1A1A1B] border-cyan-500/15 mb-1.5" data-testid={`vs-pending-${vs.id}`}>
                  <CardContent className="p-2.5 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">{vs.creator_nickname} — Score: {vs.creator_score}</p>
                    </div>
                    <Button size="sm" className="bg-cyan-600 hover:bg-cyan-700 text-[10px] h-7" onClick={() => joinVs(vs)} data-testid={`vs-join-${vs.id}`}>Accetta</Button>
                  </CardContent>
                </Card>
              ))}
            </TabsContent>
            ...
            <TabsContent value="history">
              {myVs.length === 0 ? (
                <Card className="bg-[#1A1A1B] border-gray-800"><CardContent className="p-6 text-center text-gray-500">
                  <p className="text-xs">Nessuna sfida</p>
                </CardContent></Card>
                    ...
                    <CardContent className="p-2 flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-1">
                          ...
                          <Badge className={`text-[8px] px-0.5 h-3.5 ${
                        ...
                        {vs.status === 'completed' && oppName && <p className="text-[9px] text-gray-500">vs {oppName} | {myScore ?? '?'}-{oppScore ?? '?'}</p>}
                        {vs.status === 'waiting' && <p className="text-[9px] text-gray-500">Score: {myScore}</p>}
                      </div>
                    </CardContent>
                  </Card>
                );
        ...
        <TabsContent value="classifica">
          <div className="space-y-2">
            <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
              <Button size="sm" variant={lbGame === 'global' ? 'default' : 'outline'}
                className="text-[9px] h-6 shrink-0 px-2" onClick={() => { setLbGame('global'); loadGlobalLb(); }} data-testid="lb-global">Globale</Button>
              {games.slice(0, 8).map(g => (
                <Button key={g.id} size="sm" variant={lbGame === g.id ? 'default' : 'outline'}
                  className="text-[9px] h-6 shrink-0 border-gray-700 px-2"
                  onClick={() => { setLbGame(g.id); loadLeaderboard(g.id, lbPeriod); }}>{g.name}</Button>
              ))}
            </div>
            {lbGame !== 'global' && (
            ...
            <div className="space-y-0.5">
              {(lbGame === 'global' ? globalLb : leaderboard).length === 0 ? (
                <Card className="bg-[#1A1A1B] border-gray-800"><CardContent className="p-5 text-center text-gray-500">
                  <Crown className="w-6 h-6 mx-auto mb-1 opacity-30" /><p className="text-xs">Nessun dato</p>
                </CardContent></Card>
                ...
                <Card key={entry.user_id || i} className={`bg-[#1A1A1B] ${entry.user_id === user?.id ? 'border-cyan-500/30' : 'border-gray-800/50'}`}>
                  <CardContent className="p-1.5 flex items-center gap-2">
                    ...
                    <p className={`flex-1 text-xs font-semibold truncate ${entry.user_id === user?.id ? 'text-cyan-400' : ''}`}>{entry.nickname}</p>
                    <div className="text-right shrink-0">
                      <p className="text-xs font-bold text-yellow-400">{lbGame === 'global' ? entry.total_score : entry.best}</p>
                      <p className="text-[8px] text-gray-500">{lbGame === 'global' ? `${entry.unique_games}g` : `${entry.plays}x`}</p>
                    </div>
                  </CardContent>
                </Card>
              ))}
            ...
            <Card className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border-cyan-500/20">
              <CardContent className="p-2.5">
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div>
                    <p className="text-lg font-black text-cyan-400">{Object.keys(stats).length}</p>
                    <p className="text-[8px] text-gray-400">Giochi</p>
                  </div>
                  <div>
                    <p className="text-lg font-black text-yellow-400">{Object.values(stats).reduce((s, g) => s + g.plays, 0)}</p>
                    <p className="text-[8px] text-gray-400">Partite</p>
                  </div>
                  <div>
                    <p className="text-lg font-black text-orange-400">{streak.wins}</p>
                    <p className="text-[8px] text-gray-400">VS Vinte</p>
                  </div>
                  <div>
                    <p className="text-lg font-black text-purple-400">{titles.length}</p>
                    <p className="text-[8px] text-gray-400">Titoli</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {st ? (
                        <div className="flex gap-2 text-[9px]">
                          <span className="text-yellow-400">Best: {st.best}</span>
                          <span className="text-gray-400">Avg: {st.avg}</span>
                          <span className="text-gray-500">{st.plays}x</span>
                        </div>
                      ) : <p className="text-[9px] text-gray-600">Mai giocato</p>}
                    </div>
                    <Button size="sm" variant="ghost" className="h-6 text-[9px] text-cyan-400 px-2"
                      onClick={() => { startSolo(g.id); setTab('solo'); }}>
